...
public readonly struct SgeGaugeSnapshot(byte addersgall, byte addersting, int addersgallTimerMs)
{
    ...
    public bool OvercapWithinMs(int thresholdMs)
        => Addersgall == 2 && ElapsedInWindowMs >= 0 && ElapsedInWindowMs >= thresholdMs;

    public bool OvercapSoon => OvercapWithinMs(19_000);
}
...
public static class SgeGaugeReader
{
    ...
    {
        var cache = Plugin.GameSnapshot;
        if (cache == null)
            return default;
        return cache.TryReadGauge<SgeGaugeSnapshot>(out var snap) ? snap : default;
    }
}
