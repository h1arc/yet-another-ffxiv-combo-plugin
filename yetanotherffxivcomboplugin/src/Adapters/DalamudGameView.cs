...
namespace yetanotherffxivcomboplugin.src.Adapters;

/// <summary>
/// Concrete IGameView backed by Dalamud services exposed by Plugin.
/// This isolates all direct API calls from the engine.
/// </summary>
internal sealed class DalamudGameView(IClientState clientState, ITargetManager targets, IPartyList party, IObjectTable objects, ICondition condition) : IGameView
{
    private readonly IClientState _clientState = clientState;
    private readonly ITargetManager _targets = targets;
    private readonly IPartyList _party = party;
    private readonly IObjectTable _objects = objects;
    ...
    public unsafe bool IsActionReady(int actionId)
    {
        ...
        // Probe both Action (GCD/spell) and Ability (oGCD) status flags.
        var actType = FFXIVClientStructs.FFXIV.Client.Game.ActionType.Action;
        var ablType = FFXIVClientStructs.FFXIV.Client.Game.ActionType.Ability;
        var stActRaw = am->GetActionStatus(actType, (uint)actionId);
        var stAblRaw = am->GetActionStatus(ablType, (uint)actionId);
        bool stAct = stActRaw == 0;
        bool stAbl = stAblRaw == 0;

        // Recast remaining (take max of both categories; ability recast usually drives oGCDs like Assize).
        float tAct = am->GetRecastTime(actType, (uint)actionId);
        float eAct = am->GetRecastTimeElapsed(actType, (uint)actionId);
        float remAct = MathF.Max(0f, tAct - eAct);
        float tAbl = am->GetRecastTime(ablType, (uint)actionId);
        float eAbl = am->GetRecastTimeElapsed(ablType, (uint)actionId);
        float remAbl = MathF.Max(0f, tAbl - eAbl);
        float remaining = MathF.Max(remAct, remAbl);

        // Heuristic: some oGCDs incorrectly appear usable via Action status path when only the Ability recast matters.
        // If only one path reports usable but there is significant remaining recast, treat as not ready.
        if (remaining > 0.25f && (stAct ^ stAbl))
        {
            // Mismatched status with real remaining cooldown: not ready.
            return false;
        }

        // If either path reports usable and no significant remaining recast, it's ready.
        if ((stAct || stAbl) && remaining <= 0.25f)
            return true;

        // If neither path reports usable, only ready when remaining practically zero.
    ...
    public bool TryGetHitboxRadius(ulong objectId, out float radius)
    {
        radius = 0f;
        ...
        {
            if (o is not ICharacter ch) continue;
            if (o.GameObjectId != objectId) continue;
            // Dalamud ICharacter exposes HitboxRadius
            radius = ch.HitboxRadius;
